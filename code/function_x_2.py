import sys
# nm = input()
# n = int(nm.split()[0])
# m = int(nm.split()[1])
#
# ss = sys.stdin.readlines()
m = 1000
ss = """7 6429964 4173738 9941618 2744666 5392018 5813128 9452095
7 6517823 4135421 6418713 9924958 9370532 7940650 2027017
7 1506500 3460933 1550284 3679489 4538773 5216621 5645660
7 7443563 5181142 8804416 8726696 5358847 7155276 4433125
7 2230555 3920370 7851992 1176871 610460 309961 3921536
7 8518829 8639441 3373630 5036651 5291213 2308694 7477960
7 7178097 249343 9504976 8684596 6226627 1055259 4880436"""

ss = """1 2 3
1 2 3
1 2 3"""
m=998
ss="""6 67828645 425092764 242723908 669696211 501122842 438815206
4 625649397 295060482 262686951 815352670
3 100876777 196900030 523615865"""
m=1000
ss="""2 5 4
3 7 8 9
5 5 7 8 9 10"""
m=24
ss="""3 24 48 96
4 24 48 96 24"""
ss = ss.split("\n")
print(ss)

inps = []
for ar in ss:
    #print(ar)
    ar2 = [int(x)*int(x) for x in ar.split()[1:]]
    inps.append(ar2)
    print("AR2",ar2)

rez0 = inps[0]
for inp_index in range(1, len(inps)):
    a = inps[inp_index]
    rez = []
    for element in rez0:
        for element2 in a:
            rez.append(element+element2)
    rez0 = rez
    print("rez0", rez)
print("rez2",rez0)
r = [x%m for x in rez0]
print("R",r)
print(sorted(r,reverse=True))
print("rez3",max(r))